...
import {Panel,Breadcrumb,Form,FormControl,Label,Icon,Button,Radio,LoadingState} from 'tinper-bee';
import UploadFile from '../../../components/UploadFile';
import { IPageDetailProps, IPageDetailState } from '../../../services/Model/Models';
import { convertFiles, getValidateFieldsTrim } from '../../../utils/tools';
...
class ManTrans extends React.Component<IPageProps,IPageState> {
    ...
    componentDidMount() {
        if(this.isPage()){
            ...
            //in dailog
            const m1=new RegExp('/process-trans/:id'.replace(':id','\w?'));
    ...
    render() {
        ...
        <FormItem>
                <Label>申请戒毒执行地</Label>
                <FormControl placeholder="请输入申请社区戒毒执行地"
                            {...getFieldProps('toCommunity', {
                                validateTrigger: 'onBlur',
                                rules: [{
                                    required: true, message: <span><Icon type="uf-exc-t"></Icon><span>请输入申请社区戒毒执行地</span></span>,
                                }],
                            }) }
                />
                <span className='error'>
                     {getFieldError('toCommunity')}
                </span>
        </FormItem>
        <FormItem>
        ...
        <FormItem>
                <Label>变更后实际详址</Label>
                <FormControl placeholder="请输入变更后实际详址"
                            {...getFieldProps('address', {
                                validateTrigger: 'onBlur',
                                rules: [{
                                    required: true, message: <span><Icon type="uf-exc-t"></Icon><span>请输入变更后实际详址</span></span>,
                                }],
                            }) }
                />
                <span className='error'>
...
export default Form.createForm()(ManTrans)